...
def test_perfect_entry_requires_edge_when_enabled():
    book_obj = book(0.03)
    executions = {
        ContractSide.YES: estimate_buy_execution(book_obj, ContractSide.YES, 1),
        ContractSide.NO: estimate_buy_execution(book_obj, ContractSide.NO, 1),
...
def test_follows_favorite_without_model_direction_match():
    book_obj = book(0.03)
    executions = {
        ContractSide.YES: estimate_buy_execution(book_obj, ContractSide.YES, 1),
        ContractSide.NO: estimate_buy_execution(book_obj, ContractSide.NO, 1),
    ...
    ctx = resolve_longshot_entries(
        ...
        forecast=forecast(0.80),
        seconds_remaining=120,
        cfg=LongshotConfig(enabled=True, perfect_entry_only=False),
    )
    assert ctx.forced_side is ContractSide.NO
    assert not any(
        f.reason == "model direction does not match the market favorite"
